...
/**
 * Material 3 皮肤的"大标题 + 搜索栏"顶栏 —— 逐字移植自 KernelSU
 * `ui/component/material/SearchBar.kt`（仅把自带的 SnackBarHost 换成 M3
 * SnackbarHost，并在无 snackbar 需求时不渲染）。
 *
 * 折叠态是 `LargeFlexibleTopAppBar` + 下方 ContainedSearchBar；点击搜索条后
 * `ExpandedFullScreenContainedSearchBar` 接管整屏，[defaultContent] 渲染未输入
 * 时的页面、[searchContent] 渲染结果区，二者共用同一个列表实现。
 */
@Composable
fun SearchAppBar(
        ...
        snapshotFlow { searchBarState.currentValue }
            ...
            .collect { value ->
                val collapsedFromExpanded =
                    previousSearchBarValue != SearchBarValue.Collapsed &&
                ...
                leadingIcon = {
                    if (isSearchExpanded) {
                        IconButton(
                            ...
                            Icon(Icons.AutoMirrored.Rounded.ArrowBack, "收起搜索")
                        }
                    } else {
                ...
                trailingIcon = {
                    if (isSearchExpanded && currentQuery.isNotEmpty()) {
                        IconButton(
                            ...
                            content = { Icon(Icons.Filled.Close, "清空") },
                        )
                    }
                },
